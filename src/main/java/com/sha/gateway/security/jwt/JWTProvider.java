...
public class JWTProvider implements JWTProvidable
{
    ...
    //**********2
    /*
     * Java sevurity için özel ve genel anahtar kodlayıcıya ihtiyaç var.
     * özel anahtar kodlama için PKCS8EncodedKeySpec sinifi
     * açık anahtar kodlama için X509EncodedKeySpec sinifi
     * */
 ...
 */
    ...
    public String generateToken(UserPrinciple authentication)
    {
        ...
        // talep metodu (claim())
        return Jwts.builder().setSubject(authentication.getUsername()).claim("id", authentication.getId())
                .claim("roles", authorities)
                .setExpiration(new Date(System.currentTimeMillis() + JWT_EXPIRATION_MS))// su anki zaman + 2 gun
